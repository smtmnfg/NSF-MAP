...
final_original_time_series_list = []
final_labels_list = []
final_outputs_list = []
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for i, (time_series, labels, index) in enumerate(val_loader):
            # Move data to device
            time_series = time_series.to(device)
            labels = labels.to(device)
...
df.to_csv('df_50_epochs_frozen_encoder_weighted_loss_optimized4.csv', index=False)

torch.save(model.state_dict(), "decison_level_fusion_with pretrained_cnn_new_frozen_encoder_weighted_loss_epoch_50_optimized4.pth")
